...
function dirWeightBytes(path: string): number {
  ...
  const walk = (dir: string) => {
    for (const ent of readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (/\.(safetensors|npz|bin|pt)$/i.test(ent.name)) total += statSync(p).size;
    }
  };
  try {
    walk(path);
...
/** Unified-memory estimate: weight files + HF config KV. */
export const mlxMemoryEstimator: MemoryEstimator = {
  async estimate(req: MemoryEstimateRequest): Promise<MemoryEstimateResult> {
    ...
    const hidden = Number(config?.hidden_size ?? config?.n_embd ?? 2048);
    const layers = Number(config?.num_hidden_layers ?? config?.n_layer ?? 16);
    const kvHeads = Number(config?.num_key_value_heads ?? config?.num_attention_heads ?? 8);
    const heads = Number(config?.num_attention_heads ?? 16);
    ...
    const fileBytes = weightBytes > 0 ? weightBytes : Math.max(1.2 * 1024 ** 3, 1);
